...
sealed abstract class RaiseHandler[-A, -S](runtime: Runtime[Executable.BaseEnv]) { self =>
  ...
  protected def handleRaise(raise: Raise[A, S]): SKTask[Unit]
  ...
  inline final def raiseZIO(raise: SKTask[Raise[A, S]]): Unit = raiseManyZIO(raise.map(_ :: Nil))
  final def raiseManyZIO(raise: SKTask[List[Raise[A, S]]]): Unit =
    runtime.unsafeRun {
      raise
        .flatMap {
  ...
  inline final def raiseActionZIO(raise: SKTask[A]): Unit = raiseZIO(raise.map(Raise.Action(_)))
  inline final def raiseActionManyZIO(raise: SKTask[List[A]]): Unit = raiseManyZIO(raise.map(_.map(Raise.Action(_))))

  // =====| Helpers |=====

  private[web] final def mapRaise[NewA, NewS](f: Raise[NewA, NewS] => SKTask[List[Raise[A, S]]]): RaiseHandler[NewA, NewS] =
  ...
  @nowarn
  private[web] final def mapAction[NewA](f: NewA => SKTask[List[Raise[A, S]]]): RaiseHandler[NewA, S] =
    mapRaise[NewA, S] {
...
object RaiseHandler {
  ...
  private[web] def root[A, S](
    ...
    new RaiseHandler[A, S](runtime) {
      ...
      override protected def handleRaise(raise: Raise[A, S]): SKTask[Unit] = {
        @nowarn
        def handleStandardOrUpdate(standardOrUpdate: Raise.StandardOrUpdate[S]): SKTask[Unit] =
              ...
              standard match {
                ...
                case history: Raise.Standard.History =>
                  history match {
                    ...
                    case Raise.Standard.History.Go(_)         => ZIO.failNEL(KError.???("History.go")) // TODO: I believe this needs to have access to the RouteMatcher
                  }
